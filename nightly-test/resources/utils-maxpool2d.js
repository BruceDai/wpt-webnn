...
const loadTests = (operationName) => {
  ...
  const json = loadJSON(`./resources/test_data/max_pool2d-repro.json`);
  ...
  if (Array.isArray(resources.inputs)) {
    ...
    for (let subInput of resources.inputs) {
      if (!subInput.hasOwnProperty('constant') || !subInput.constant) {
        inputs[subInput.name] = getTypedArrayData(subInput.type, subInput.data); // new TypedArrayDict[subInput.type](subInput.data);
      }
    }
  } else {
    for (let inputName in resources.inputs) {
      ...
      if (!subTestByName.hasOwnProperty('constant') || !subTestByName.constant) {
        inputs[inputName] = getTypedArrayData(subTestByName.type, subTestByName.data); // new TypedArrayDict[subTestByName.type](subTestByName.data);
      }
    }
  }
  let outputs = {};
