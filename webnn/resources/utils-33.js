  ...
  if (navigator.ml) {
    ...
    promise_setup(() => {
      return new Promise(async (resolve) => {
        // Create context for checking test whether be supported by context.
        const context = await getContext();
        minimumDataTypeSet = await getMinimumDataTypeSetJson();
        await Promise.all([context, minimumDataTypeSet]);
        tests.filter(isTargetTest).forEach((test) => {
          if (validateContextSupportsGraph(context, test.graph) ||
              isMinimumTest(test)) {
            testsToRun.push(test);
          } else {
            // Skip optional test that isn't supported by the context
    ...
    promise_test(async () => {
      await Promise.all(testsToRun.map((test) => {
        promise_test(async () => {
